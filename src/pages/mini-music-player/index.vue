<template>
  <div class="wrapper">
    ...
    <div class="list">
      <input v-model="loginParam.phone" type="number" />
      <input v-model="loginParam.password" type="password" />
...
export default {
  ...
  methods: {
    async login() {
      await musicApi.loginMusic(this.loginParam);
    },
    /** 播放/暂停 */
    play() {
    ...
    async getRecommendList() {
      const { code, status, data, message } = await musicApi.getRecommendList();
      console.log(data);
      this.tracks = data.dailySongs.map((item, index) => {
        return {
          id: index,
          name: item.name,
