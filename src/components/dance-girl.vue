...
export default {
  ...
  mounted() {
    const danceGirl = document.getElementsByClassName('dance-girl')[0];
    ...
    const img = new Image();
    img.src = imgUrl;
    img.complete ? (img.onload = onImgLoaded) : (img.onload = onImgLoaded);

    function onImgLoaded() {
      /** 图片的宽高比 */
      const imgRatio = img.width / img.height;
      /** 图片盒子的宽 */
      let imgWidth = danceGirl.offsetWidth;
      /** 图片盒子的高 */
      let imgHeight = imgWidth / imgRatio;

      if (imgHeight > danceGirl.offsetHeight) {
        imgHeight = danceGirl.offsetHeight;
